...
export default function InstallSection() {
  ...
  const CMDS: Record<OS, string> = {
    unix:    "curl -fsSL https://tsuki.sh/install.sh | sh",
    windows: "irm https://tsuki.sh/install.bat | iex",
  };
        ...
        <div className="reveal" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 80, alignItems: "start" }}>
          ...
          <div>
            ...
            <p className="t-body" style={{ marginBottom: 32 }}>
              The installer detects your OS and CPU architecture, downloads the
              latest release from GitHub, and puts the tsuki binary on your PATH.
            </p>
            ...
            <div style={{ display: "flex", gap: 4, padding: 4, background: "#111", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 8, width: "fit-content", marginBottom: 16 }}>
              {(["unix","windows"] as OS[]).map(o => (
                <button key={o} onClick={() => { setOs(o); setCopied(false); }} style={{ padding: "6px 16px", background: os === o ? "rgba(255,255,255,0.08)" : "transparent", border: "1px solid", borderColor: os === o ? "rgba(255,255,255,0.12)" : "transparent", borderRadius: 5, fontFamily: "var(--f-mono)", fontSize: 11, letterSpacing: "0.04em", color: os === o ? "#fff" : "#555", cursor: "pointer", transition: "all 0.15s ease" }}>
            ...
            <button onClick={copy} className="cmd-box" style={{ width: "100%", marginBottom: 10 }}>
              ...
              <span style={{ padding: "3px 10px", background: copied ? "rgba(0,229,176,0.1)" : "rgba(255,255,255,0.04)", border: "1px solid", borderColor: copied ? "rgba(0,229,176,0.3)" : "rgba(255,255,255,0.07)", borderRadius: 4, fontFamily: "var(--f-mono)", fontSize: 10, letterSpacing: "0.06em", color: copied ? "var(--accent)" : "#333", transition: "all 0.2s ease", flexShrink: 0 }}>
                {copied ? "Copied!" : "Copy"}
              </span>
            </button>
